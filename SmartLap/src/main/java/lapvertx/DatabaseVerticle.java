...
public class DatabaseVerticle extends AbstractVerticle {
	...
	private void getpisodondehacecalor(RoutingContext routingContext) {
		...
		mySQLPool.query(
				"SELECT  sensores.*  FROM lap.sensores,  dispositivo  where  dispositivo.idDispositivo=sensores.idDispositivo and idEdificio="
						+ routingContext.request().getParam("IdEdificio") + " and location="
						+ routingContext.request().getParam("location")
						+ " and temperatura> 28 group by location order by timestamp desc limit 100"
					...
					if (res.succeeded()) {
						...
						for (Row row : resultSet) {
							...
							result.add(JsonObject.mapFrom(new Sensores(row.getInteger("idSensor"),
									row.getInteger("idDispositivo"), row.getFloat("temperatura"),

									row.getFloat("humedad"), row.getInteger("calidad_aire"), row.getLong("timestamp"),
	...
	private void getpisodondehacefrio(RoutingContext routingContext) {
		...
		mySQLPool.query(
				"SELECT  sensores.*  FROM lap.sensores,  dispositivo  where  dispositivo.idDispositivo=sensores.idDispositivo and idEdificio="
						+ routingContext.request().getParam("IdEdificio") + " and location="
						+ routingContext.request().getParam("location")
						+ " and temperatura< 26 group by location order by timestamp desc limit 100"
						+ routingContext.request().getParam("idEdificio")
						+ routingContext.request().getParam("location"),
				res -> {
					if (res.succeeded()) {
	...
	private void getValueByDispositivo(RoutingContext routingContext) {
		mySQLPool.query(
				"SELECT * FROM lap.sensores WHERE idDispositivo= " + routingContext.request().getParam("idDispositivo"),
				res -> {
					if (res.succeeded()) {
						RowSet<Row> resultSet = res.result();
						System.out.println("El número de elementos obtenidos es " + resultSet.size());
	...
	private void getpisoscalor(RoutingContext routingContext) {
		mySQLPool.query(
				"SELECT * FROM lap.sensores , lap.edificio, lap.dispositivo WHERE temperatura > 26 and edificio.idEdificio= "
	...
	private void getpisosdondehacefrio(RoutingContext routingContext) {
		...
		mySQLPool.query(
				"SELECT  sensores.*  FROM lap.sensores,  dispositivo  where  dispositivo.idDispositivo=sensores.idDispositivo and idEdificio="
						+ routingContext.request().getParam("IdEdificio")
						+ "  and temperatura< 20 group by location order by timestamp desc limit 100"
						+ routingContext.request().getParam("idEdificio"),
				res -> {
					if (res.succeeded()) {
						RowSet<Row> resultSet = res.result();
	...
	private void getValueByPiso(RoutingContext routingContext) {
		mySQLPool.query(
				"SELECT * FROM sensores sensores,dispositivo " + " where dispositivo.idEdificio="
						+ routingContext.request().getParam("idEdificio") + " and sensores.location="
						+ routingContext.request().getParam("location") + " group by location order by timestamp desc",
				res -> {
					if (res.succeeded()) {
						RowSet<Row> resultSet = res.result();
						System.out.println("El número de elementos obtenidos es " + resultSet.size());
	...
	private void getValueByEdificio(RoutingContext routingContext) {
		mySQLPool.query(
				"select * from sensores,dispositivo where sensores.idDispositivo=dispositivo.idDispositivo and dispositivo.idEdificio="
						+ routingContext.request().getParam("idEdificio")
						+ "  group by location order by timestamp desc",
				res -> {
					if (res.succeeded()) {
						RowSet<Row> resultSet = res.result();
						System.out.println("El número de elementos obtenidos es " + resultSet.size());
	...
	private void deleteEdificio(RoutingContext routingContext) {
		mySQLPool.query("DELETE  FROM lap.edificio WHERE edificio.idEdificio="
				+ routingContext.request().getParam("idEdificio"), res -> {
					if (res.succeeded()) {
						System.out.println(
								"Se ha borrado el edificio " + routingContext.request().getParam("idEdificio") + ".");
						routingContext.response().setStatusCode(200).putHeader("content-type", "application/json")
								.end();
					} else {
						routingContext.response().setStatusCode(401).putHeader("content-type", "application/json")
